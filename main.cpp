...
void test() {
    global_sequence->synchronize(); // 1

    int new_id = global_sequence->before_thread_start(); // 2

    std::thread t(
        [](int id){
            global_sequence->thread_begin(id); // 2

            global_sequence->thread_end(); // 3
        }, new_id
    );
    global_sequence->thread_start(); // 2

    global_sequence->join(); // 3

    t.join();
}

int main() {
    global_sequence = new sequence;

    do {
        test();
    } while (global_sequence->next_sequence());
}
